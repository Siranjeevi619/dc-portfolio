...
export function Publications() {
  ...
  const canScrollNext = selectedIndex < publicationsData.length - 1

  const scrollPrev = () => setSelectedIndex((prev) => Math.max(0, prev - 1))
  const scrollNext = () => setSelectedIndex((prev) => Math.min(publicationsData.length - 1, prev + 1))
  ...
  return (
    <section id="publications" className="py-24 px-4 sm:px-6 lg:px-8 bg-muted/30">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center justify-between mb-10 gap-8">
          <ScrollAnimation>
            ...
            <p className="text-muted-foreground text-lg mt-4 italic max-w-2xl mx-auto">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="p-3 rounded-full border border-border bg-card hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-md hover:scale-110 active:scale-95 group"
        ...
        {/* Year Quick Picker */}
        <div className="flex flex-wrap justify-center gap-2 mb-8 items-center bg-card/60 backdrop-blur-md p-2.5 rounded-full border border-border w-fit mx-auto shadow-lg relative z-10">
        ...
        {/* Active Year Content (Dynamic Height) */}
        <div 
          key={selectedIndex} 
          className="animate-in fade-in slide-in-from-bottom-6 duration-700 ease-in-out"
        >
          {publicationsData[selectedIndex] && (
            <div className="space-y-4">
              <div className="flex items-center gap-4 mb-4">
                <div className="px-4 py-1.5 bg-primary/10 text-primary rounded-full text-lg font-bold">
                  {publicationsData[selectedIndex].year}
                </div>
                <div className="h-px bg-border flex-1"></div>
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest bg-muted px-3 py-1 rounded-lg">
                  {publicationsData[selectedIndex].publications.length} Papers
                </span>
              </div>
              
              <div className="flex flex-wrap justify-center gap-6">
                {publicationsData[selectedIndex].publications.map((pub, pIdx) => (
                  <div
                    key={pIdx}
                    className="bg-card p-6 rounded-2xl border border-border hover:border-primary/40 transition-all group relative overflow-hidden flex flex-col justify-between w-full md:w-[calc(50%-0.75rem)] lg:max-w-2xl shadow-sm hover:shadow-md"
                  >
                    <div className="space-y-4">
